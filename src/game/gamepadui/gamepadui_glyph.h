...
public:
    ...
    bool SetupGlyph( int nSize, const char *pszAction, bool bBaseLight = false )
    {
        ...
        uint64 nController = SteamInput()->GetControllerForGamepadIndex( 0 );
        if ( !nController )
        ...
        for ( int i = 0; i < 2; i++ )
        {
            ...
            g_pMatSystemSurface->DrawSetTextureRGBAEx2( m_nOriginTextures[i], bitmap.GetBits(), bitmap.Width(), bitmap.Height(), ImageFormat::IMAGE_FORMAT_RGBA8888, true );
        }
